...
在集群模式下，Quartz通过数据库行                          锁来确保同一个任务不会被多个节点同时执行。当一个调度器实例尝试执行一个任务时，它会先在数据库中对该任务加锁。如果加锁成功，该实例就会执行任务；否则，意味着有其他实例已经在执行该任务，当前实例就会放弃执行。
...
![img](https://test-fsservice.oss-cn-shanghai.aliyuncs.com/fs/test/2024/202403011742870.jpg)
...
![image-20240301113136175](/Users/cong/Library/Application Support/typora-user-images/image-20240301113136175.png)
...
| 项目\方案                                                    | qz                                                           | Xxl-job                                             | e-j  | p-j  | d-s  |
| ------------------------------------------------------------ | :----------------------------------------------------------- | --------------------------------------------------- | ---- | ---- | ---- |
| 任务分配的方式如何保证公平                                   | 抢占式，获取到数据库锁就执行，可以考虑分离长短作业           | [均衡调度](#均衡调度)                               |      |      |      |
| 每个执行器的容量上限                                         | 使用线程池来执行作业，决定于线程池的大小。                   | 线程池                                              |      |      |      |
| 当pod重启时被分配的任务会怎么处理（如果有任务正在执行时会怎么样） | 失败，会重新分配执行，wait-for-jobs-to-complete-on-shutdown: true # 应用关闭时，是否等待定时任务执行完成。默认为 false ，建议设置为 true | [过期处理策略](#过期处理策略),[灰度上线](#灰度上线) |      |      |      |
| 长事务的任务执行会不会有问题                                 | 会出现`Handling 2 trigger(s) that missed their scheduled fire-time.`任务会丢失，需要手动设置处理。 | [避免任务重复执行](#避免任务重复执行)               |      |      |      |
