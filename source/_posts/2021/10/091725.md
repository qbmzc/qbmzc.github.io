...
docker run -d -u root \
  -p 9002:8080 \
  --name code-server \
  -v /xxx/docker/vscode/config.yml:/root/.config/code-server/config.yaml \
  -v /xxx/docker/vscode:/home/code \
...
docker run -d -p 15672:15672  -p  5672:5672  -e RABBITMQ_DEFAULT_USER=cong.zheng -e RABBITMQ_DEFAULT_PASS=xxx@123 --name rabbitmq  rabbitmq:3-management
```
...
# 手动更新方式 : docker-compose pull && docker-compose up -d
  reader:
    ...
    environment:
      ...
      - READER_APP_SECUREKEY=Snow@1021  #管理员密码  建议修改
