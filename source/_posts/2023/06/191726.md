...
```dockerfile
FROM images.xxx.com/middle/wps-office:v1.3.4
MAINTAINER  cong.zheng 
...
RUN echo "Exec=xfce4-terminal -e='java -Xms1g -Xmx1g -Dspring.cloud.nacos.config.server-addr=config.xxx.com:8848 -Dspring.profiles.active=test -Dspring.cloud.nacos.config.namespace=TEST  -Dqueue=doc_convert_test  -Duser.timezone=GMT+08 -XX:+UseG1GC -XX:MaxGCPauseMillis=200 -jar /k8sapps/new-doc-convert-0.0.1-SNAPSHOT.jar '" >> /etc/xdg/autostart/new.desktop
```
...
docker login 
# 登录images.xxx.com
docker login images.xxx.com -u admin -p *****
