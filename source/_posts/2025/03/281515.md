...
在 Docker 容器中使用 `uv` 可以显著加快镜像构建速度，减少构建时间和镜像体积。以下是几种推荐的使用方式。

### 方法一：官方推荐的多阶段构建

这是最推荐的方式，利用 Docker 多阶段构建来优化镜像大小：
...
在 Docker 环境中，`uv` 相比传统 `pip` 的优势更加明显：

| 操作 | pip | uv | 提升 |
|------|-----|----|----|
| 首次构建 | 120s | 15s | **8x** |
| 缓存构建 | 45s | 3s | **15x** |
| 镜像大小 | 450MB | 380MB | **减少15%** |

### 常见问题

**Q: 如何在 Alpine Linux 中使用 uv？**
